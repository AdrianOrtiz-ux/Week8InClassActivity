...
new_youth %>% 
  ...
  nest(gender, grade) %>% #group the values together
  mutate(total = sum(n),    
         proportion = round(n/total, 4)) %>% #create new variables "total" and "proportion"
  filter(gender, grade, perception_of_weight, proportion) %>% #choose only these columns
  ...
  relocate(contains("under"), .before = 'About the right weight') %>% #_______
  relocate('Very underweight', .after = grade) #______
```
...
> No because that's all the data that we need
...
```{r}
child %>% 
  pivot_longer(cols = 1:2, names_to = "count", values_to = "n") 
```
...
```{r}
new_child %>% 
  _______(Label %in% c("Too little", "About right","Too much")) %>% #pull out just the responses listed
